...
export default function OrderModal({ product, onClose }) {
  ...
  const handleOrder = (e) => {
    ...
    const message =
      ...
      `*Price:* GH₵${product.price}%0A` +
      `*Quantity:* ${form.quantity}%0A` +
      `*Total:* GH₵${(product.price * form.quantity).toFixed(2)}%0A%0A` +
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="order-product-preview">
          ...
          <div>
            ...
            <span className="order-product-price">GH₵ {product.price}</span>
          </div>
        </div>
        ...
        <form onSubmit={handleOrder}>
          ...
          <div className="form-group">
            ...
            <input
              name="address"
              type="text"
              placeholder="Your delivery address in Accra"
              value={form.address}
              onChange={handleChange}
              required
            />
          ...
          <div className="order-total">
            <span>Total</span>
            <span className="total-amount">GH₵ {(product.price * form.quantity).toFixed(2)}</span>
          </div>

          <button type="submit" className="whatsapp-order-btn">
            <FaWhatsapp size={20} />
            Order via WhatsApp
          </button>
        </form>
