...
export default function AdminDashboard() {
  ...
  const [productForm, setProductForm] = useState({
    name: '', category: 'wigs', description: '', price: '',
    in_stock: true, featured: false, images: []
  })

  // Service form
  const [serviceForm, setServiceForm] = useState({
    name: '', description: '', show_on_homepage: false
  })
  ...
  async function fetchAll() {
    ...
    if (set) {
      const obj = {}
      set.forEach(s => { obj[s.key] = s.value })
      setSettings(obj)
    }
  }
  ...
  const handleLogout = () => {
    sessionStorage.removeItem('ricsglam_admin')
    navigate('/')
  }
  ...
  const openProductForm = (product = null) => {
    if (product) {
      ...
      setEditingItem(null)
      setProductForm({ name: '', category: 'wigs', description: '', price: '', in_stock: true, featured: false, images: [] })
    }
  ...
  const openServiceForm = (service = null) => {
    if (service) {
      ...
      setServiceForm({ name: service.name, description: service.description || '', show_on_homepage: service.show_on_homepage })
    } else {
      setEditingItem(null)
      setServiceForm({ name: '', description: '', show_on_homepage: false })
    }
  ...
  const saveService = async (e) => {
    ...
    try {
      if (editingItem) {
        await supabase.from('services').update(serviceForm).eq('id', editingItem.id)
  ...
  const toggleServiceHomepage = async (id, current) => {
    await supabase.from('services').update({ show_on_homepage: !current }).eq('id', id)
    fetchAll()
  }
  ...
  const openTestimonialForm = (t = null) => {
    if (t) {
      setEditingItem(t)
      setTestimonialForm({ customer_name: t.customer_name, review: t.review })
    } else {
      setEditingItem(null)
      setTestimonialForm({ customer_name: '', review: '' })
    }
  ...
  const saveTestimonial = async (e) => {
    ...
    try {
      if (editingItem) {
        await supabase.from('testimonials').update(testimonialForm).eq('id', editingItem.id)
  ...
  const updateRatingStatus = async (id, status) => {
    await supabase.from('ratings').update({ status }).eq('id', id)
  ...
  const handleBannerUpload = async (e) => {
    ...
    try {
      ...
      await updateSetting('cta_banner', url)
    } catch {
      ...
      <main className="admin-main">
        ...
        {activeTab === 'products' && (
          <div className="admin-section">
            <div className="admin-section-header">
              <h2>Products</h2>
              <button className="admin-add-btn" onClick={() => openProductForm()}>
                ...
                <tbody>
                  {products.length === 0 ? (
                    ...
                    <tr key={p.id}>
                      ...
                      <td>
                        <button
                          className={`toggle-btn ${p.in_stock ? 'on' : 'off'}`}
                          onClick={() => toggleProductField(p.id, 'in_stock', p.in_stock)}
                        >
                      ...
                      <td>
                        <button
                          className={`toggle-btn ${p.featured ? 'on' : 'off'}`}
                          onClick={() => toggleProductField(p.id, 'featured', p.featured)}
                        >
                      ...
                      <td>
                        <div className="table-actions">
                          <button className="icon-btn edit" onClick={() => openProductForm(p)}><FiEdit2 size={15} /></button>
                          <button className="icon-btn delete" onClick={() => deleteProduct(p.id)}><FiTrash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              <div className="admin-modal-overlay" onClick={() => setShowProductForm(false)}>
                <div className="admin-form-modal" onClick={e => e.stopPropagation()}>
                  <div className="admin-modal-header">
                    ...
                    <button onClick={() => setShowProductForm(false)}><FiX size={18} /></button>
                  </div>
        ...
        {activeTab === 'services' && (
          <div className="admin-section">
            <div className="admin-section-header">
              <h2>Services</h2>
              <button className="admin-add-btn" onClick={() => openServiceForm()}>
                ...
                <tbody>
                  {services.length === 0 ? (
                    ...
                    <tr key={s.id}>
                      ...
                      <td>
                        <button
                          className={`toggle-btn ${s.show_on_homepage ? 'on' : 'off'}`}
                          onClick={() => toggleServiceHomepage(s.id, s.show_on_homepage)}
                        >
                          {s.show_on_homepage ? 'Visible' : 'Hidden'}
                        </button>
                      </td>
                      <td>
                        <div className="table-actions">
                          <button className="icon-btn edit" onClick={() => openServiceForm(s)}><FiEdit2 size={15} /></button>
                          <button className="icon-btn delete" onClick={() => deleteService(s.id)}><FiTrash2 size={15} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              <div className="admin-modal-overlay" onClick={() => setShowServiceForm(false)}>
                <div className="admin-form-modal small" onClick={e => e.stopPropagation()}>
                  <div className="admin-modal-header">
                    ...
                    <button onClick={() => setShowServiceForm(false)}><FiX size={18} /></button>
                  </div>
                  <form onSubmit={saveService}>
                    ...
                    <div className="form-group">
                      <label className="toggle-label">
                        <input
                          type="checkbox"
                          checked={serviceForm.show_on_homepage}
                          onChange={e => setServiceForm({ ...serviceForm, show_on_homepage: e.target.checked })}
                        />
                        Show on Homepage
                      </label>
                    </div>
                    <div className="admin-form-actions">
                      <button type="button" className="btn-cancel" onClick={() => setShowServiceForm(false)}>Cancel</button>
                      <button type="submit" className="btn-save">{editingItem ? 'Update' : 'Add Service'}</button>
                    </div>
                  </form>
                </div>
              </div>
        ...
        {activeTab === 'testimonials' && (
          <div className="admin-section">
            <div className="admin-section-header">
              <h2>Testimonials</h2>
              <button className="admin-add-btn" onClick={() => openTestimonialForm()}>
            ...
            <div className="testimonials-admin-grid">
              {testimonials.length === 0 ? (
                <p className="empty-msg">No testimonials yet.</p>
              ) : testimonials.map(t => (
                <div key={t.id} className="testimonial-admin-card">
                  <p>"{t.review}"</p>
                  <span>— {t.customer_name}</span>
                  <div className="table-actions">
                    <button className="icon-btn edit" onClick={() => openTestimonialForm(t)}><FiEdit2 size={15} /></button>
                    <button className="icon-btn delete" onClick={() => deleteTestimonial(t.id)}><FiTrash2 size={15} /></button>
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="admin-modal-overlay" onClick={() => setShowTestimonialForm(false)}>
                <div className="admin-form-modal small" onClick={e => e.stopPropagation()}>
                  <div className="admin-modal-header">
                    ...
                    <button onClick={() => setShowTestimonialForm(false)}><FiX size={18} /></button>
                  </div>
                  <form onSubmit={saveTestimonial}>
                    ...
                    <div className="admin-form-actions">
                      <button type="button" className="btn-cancel" onClick={() => setShowTestimonialForm(false)}>Cancel</button>
                      <button type="submit" className="btn-save">{editingItem ? 'Update' : 'Add Testimonial'}</button>
                    </div>
                  </form>
                </div>
              </div>
        ...
        {activeTab === 'ratings' && (
          <div className="admin-section">
            <div className="admin-section-header">
              <h2>Ratings</h2>
              <span className="pending-badge">{pendingRatings.length} pending</span>
            </div>
            ...
            <div className="ratings-filter-tabs">
              {['all', 'pending', 'approved', 'rejected'].map(status => (
                <button
                  key={status}
                  className={`ratings-tab ${ratingsFilter === status ? 'active' : ''}`}
                  onClick={() => setRatingsFilter(status)}
                >
            ...
            <div className="admin-table-wrap">
              <table className="admin-table">
                ...
                <tbody>
                  {ratings.filter(r => ratingsFilter === 'all' || r.status === ratingsFilter).length === 0 ? (
                    <tr><td colSpan={5} className="empty-row">No ratings found.</td></tr>
                  ) : ratings.filter(r => ratingsFilter === 'all' || r.status === ratingsFilter).map(r => (
                    <tr key={r.id}>
                      ...
                      <td>
                        <div className="table-actions">
                          {r.status !== 'approved' && (
                            <button className="icon-btn approve" onClick={() => updateRatingStatus(r.id, 'approved')} title="Approve">
                              <FiCheck size={15} />
                            </button>
                          )}
                          {r.status !== 'rejected' && (
                            <button className="icon-btn delete" onClick={() => updateRatingStatus(r.id, 'rejected')} title="Reject">
                              <FiX size={15} />
                            </button>
                          )}
                        </div>
          ...
          <div className="admin-section">
            ...
            <div className="settings-grid">
              ...
              <div className="settings-card">
                ...
                {settings.cta_banner && (
                  <img src={settings.cta_banner} alt="CTA Banner" className="banner-preview" />
                )}
                <label className="upload-area">
                  <FiUpload size={18} />
                  <span>{uploading ? 'Uploading...' : 'Upload new banner image'}</span>
                  <input
                    type="file"
                    accept="image/*"
